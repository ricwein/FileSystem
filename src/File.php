...
class File extends FileSystem
{
    ...
    protected function checkFileReadPermissions(): void
    {
        if (!$this->isFile() || !$this->isReadable()) {
            throw new FileNotFoundException(sprintf('unable to open file: "%s"', $this->storage instanceof Storage\Disk ? $this->storage->getPath()->getRealOrRawPath() : $this->storage::class), 404);
        }
    ...
    protected function checkFileWritePermissions(): void
    {
        if (!$this->storage->doesSatisfyConstraints()) {
            throw new AccessDeniedException(
                sprintf('unable to write file: "%s"', $this->storage instanceof Storage\Disk ? $this->storage->getPath()->getRawPath() : $this->storage::class), 403, $this->storage->getConstraintViolations()
            );
        }

        if ($this->isFile() && !$this->isWriteable()) {
            throw new AccessDeniedException(sprintf('unable to write file: "%s"', $this->storage instanceof Storage\Disk ? $this->storage->getPath()->getRawPath() : $this->storage::class), 403);
        }
    }
    ...
    public function remove(): static
    {
        ...
        // validate constraints
        if (!$this->isFile()) {
            throw new FileNotFoundException(sprintf('unable to open file: "%s"', $this->storage instanceof Storage\Disk ? $this->storage->getPath()->getRawPath() : $this->storage::class), 404);
        }
